...
# 由ravel()展平的数组元素
# unique（）保留数组中不同的值，返回两个参数 有序的保留不同的值， 每个元素出现的起始位置
...
#mlp = MLPClassifier(random_state = 1, early_stopping = True, hidden_layer_sizes = 25, verbose=True)
clf = pipeline.Pipeline([('rfr', rfr)])
#clf = pipeline.Pipeline([('mlp', mlp)])
param_grid = {'rfr__n_estimators' : list(range(109,110,1)),# 300 top
              'rfr__max_depth': list(range(3,8,1)), #list(range(7,8,1))
            }
