...
print("---  Files Loaded: %s minutes ---" % ((time.time() - start_time)/60))
...
print("--- Prepared Data: %s minutes ---" % ((time.time() - start_time)/60))
...
model = grid_search.GridSearchCV(estimator = clf, param_grid = param_grid, n_jobs = -1, cv = 10, verbose = 5, scoring=RMSE)
